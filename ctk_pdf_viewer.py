...
Widget reutilizable basado en CustomTkinter que muestra PDF página a página.
Se apoya en PyMuPDF (`fitz`) para rasterizar y en PIL para construir las
imágenes que luego se incrustan dentro de un `CTkScrollableFrame`.
"""
import io
import math
import os
...
class CTkPDFViewer(customtkinter.CTkScrollableFrame):
    """Scrollable frame que carga un PDF y lo pinta como secuencia de imágenes."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Usamos after para no bloquear el loop principal mientras convertimos el PDF.
        self.after(250, self.start_process)

    def start_process(self):
        """Lanza la conversión en un hilo para no congelar la UI."""
        Thread(target=self.add_pages).start()

    def add_pages(self):
        """Rasteriza cada página del PDF y actualiza la barra de progreso."""
    ...
    def configure(self, **kwargs):
        """Extiende configure() para permitir recargar archivo y tamaño de páginas."""
